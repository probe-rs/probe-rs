use probe_rs::{
    Session, config::MemoryRegion, Architecture, CoreInformation, CoreRegisterAddress, CoreStatus, CoreType, MemoryInterface
};
...
impl Server {
    pub fn new(session: Session) -> Self {
        Server {
            session: Mutex::new(RefCell::new(session))
        }
    }
    fn with_session<F, R>(&self, f: F) -> Result<R> where
        F: Fn(std::cell::RefMut<'_, Session>) -> Result<R>
    {
        let lock = self.session.lock().unwrap();
        let session = lock.borrow_mut();
        f(session)
    }
}
...
impl definitions::ProbeRsServer for Server {
    fn list_cores(&self) -> Result<Vec<(usize, CoreType)>> {
        Ok(self.session.lock().unwrap().borrow().list_cores())

    }
    fn memory_map(&self) -> Result<Vec<MemoryRegion>> {
        let mut result = Vec::new();
        result.extend_from_slice(self.session.lock().unwrap().borrow().memory_map());
        Ok(result)

    }
    fn architecture(&self) -> Result<Architecture> {
        Ok(self.session.lock().unwrap().borrow().architecture())
    }
    fn core_halted(&self, core_index: usize) -> Result<bool> {
        self.with_session(|mut s| {
    ...
    fn reset_and_halt(&self, core_index: usize, timeout: Duration) -> Result<CoreInformation> {
        self.with_session(|mut s| {
            let mut core = s.core(core_index).map_err(|e| e.into_json_error())?;
            core.reset_and_halt(timeout).map_err(|e| e.into_json_error())
        })
    }
    ...
    fn write_core_reg(
        ...
        self.with_session(|mut s| {
            let mut core = s.core(core_index).map_err(|e| e.into_json_error())?;
            core.write_core_reg(address, value).map_err(|e| e.into_json_error())
        })?;
        Ok(())
    }
    fn get_available_breakpoint_units(&self, core_index: usize) -> Result<u32> {
        self.with_session(|mut s| {
            let mut core = s.core(core_index).map_err(|e| e.into_json_error())?;
            core.get_available_breakpoint_units().map_err(|e| e.into_json_error())
        })
    }
    fn set_hw_breakpoint(&self, core_index: usize, address: u32) -> Result<()> {
        self.with_session(|mut s| {
            let mut core = s.core(core_index).map_err(|e| e.into_json_error())?;
            core.set_hw_breakpoint(address).map_err(|e| e.into_json_error())
        })?;
        Ok(())
    }
    fn clear_hw_breakpoint(&self, core_index: usize, address: u32) -> Result<()> {
        self.with_session(|mut s| {
            let mut core = s.core(core_index).map_err(|e| e.into_json_error())?;
            core.clear_hw_breakpoint(address).map_err(|e| e.into_json_error())
        })?;
        Ok(())
    }
    ...
    fn read_32(&self, core_index: usize, address: u32, length: usize) -> Result<Vec<u32>> {
        self.with_session(|mut s| {
            ...
            core.read_32(address, &mut vec[..]).map_err(|e| e.into_json_error())?;
            Ok(vec)
        })
    }
    fn read_8(&self, core_index: usize, address: u32, length: usize) -> Result<Vec<u8>> {
        self.with_session(|mut s| {
            let mut core = s.core(core_index).map_err(|e| e.into_json_error())?;
            let mut vec = Vec::with_capacity(length);
            core.read_8(address, &mut vec[..]).map_err(|e| e.into_json_error())?;
            Ok(vec)
        })
    }
    fn write_word_32(&self, core_index: usize, addr: u32, data: u32) -> Result<()> {
        self.with_session(|mut s| {
            let mut core = s.core(core_index).map_err(|e| e.into_json_error())?;
            core.write_word_32(addr, data).map_err(|e| e.into_json_error())
        })?;
        Ok(())
    }
    fn write_word_8(&self, core_index: usize, addr: u32, data: u8) -> Result<()> {
        self.with_session(|mut s| {
            let mut core = s.core(core_index).map_err(|e| e.into_json_error())?;
            core.write_word_8(addr, data).map_err(|e| e.into_json_error())
        })?;
        Ok(())
    }
    fn write_32(&self, core_index: usize, addr: u32, data: Vec<u32>) -> Result<()> {
        self.with_session(|mut s| {
            let mut core = s.core(core_index).map_err(|e| e.into_json_error())?;
            core.write_32(addr, &data[..]).map_err(|e| e.into_json_error())
        })?;
        Ok(())
    }
    fn write_8(&self, core_index: usize, addr: u32, data: Vec<u8>) -> Result<()> {
        self.with_session(|mut s| {
            let mut core = s.core(core_index).map_err(|e| e.into_json_error())?;
            core.write_8(addr, &data[..]).map_err(|e| e.into_json_error())
        })?;
        Ok(())
    }
}
