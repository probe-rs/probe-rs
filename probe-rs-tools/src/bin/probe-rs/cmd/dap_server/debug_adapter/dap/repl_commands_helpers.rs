...
use super::{
    ...
    repl_commands::{self, ReplCommand, ReplHandler, repl_commands},
    repl_types::*,
...
pub(crate) async fn memory_read(
    ...
    if gdb_nuf.format_specifier == GdbFormat::Instruction {
        ...
        match target_core.core.read_8(address, &mut memory_result).await {
            Ok(()) => {
...
pub(crate) fn find_commands<'a, 'b, 'c>(
    mut repl_commands: Vec<ReplCommand<ReplHandler<'a, 'b>>>,
    command_piece: &'c str,
) -> Vec<ReplCommand<ReplHandler<'a, 'b>>> {
    repl_commands.retain(move |command| command.command.starts_with(command_piece));
    repl_commands
}
...
pub(crate) fn build_expanded_commands<'a, 'b, 'c>(
    command_filter: &'c str,
) -> (String, Vec<ReplCommand<ReplHandler<'a, 'b>>>) {
    ...
    let mut repl_commands: Vec<ReplCommand<ReplHandler>> = repl_commands();
    ...
    for (piece_idx, command_piece) in command_pieces.enumerate() {
            ...
            && piece_idx != piece_count - 1
        {
            ...
            repl_commands = parent_command.sub_commands.clone();
        } else {
