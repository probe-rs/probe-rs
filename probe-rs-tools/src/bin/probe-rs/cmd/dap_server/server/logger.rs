...
impl DebugLogger {
    ...
    pub(crate) fn new(log_file: Option<&Path>) -> Result<Self, DebuggerError> {
        // A temporary storage for tracing messages to be held,
        // will only be created if we are not using a logging to a file. In that case,
        ...
        let buffer_file_inner = if log_file.is_some() {
            None
        } else {
            // If we don't have a log file, and we can't create a buffer file, then we can't log anything,
            // so we exit early, and tell the user what went wrong.
            Some(tempfile()?)
        };
        let buffer_file = buffer_file_inner.map(Mutex::new);
    ...
    pub(crate) fn buffer_file_handle(&self) -> Option<File> {
        self.buffer_file
            .as_ref()
            .and_then(|buffer_file_mutex| buffer_file_mutex.lock().ok())
            .and_then(|buffer_file: std::sync::MutexGuard<File>| buffer_file.try_clone().ok())
    }
    ...
    pub(crate) fn flush_to_dap(
        ...
        if let Some(read_from_log) = self.buffer_file_handle() {
            ...
            let later_read_pos =
                std::io::Seek::seek(&mut truncate_file, std::io::SeekFrom::End(0))?;
            self.seek_pointer = std::io::SeekFrom::Start(later_read_pos);
        } else {
            eprintln!("Failed to get a lock on the buffer file.");
        }
        Ok(())
    }

    /// Flush the buffer to the stderr
    pub(crate) fn flush(&mut self) -> Result<(), DebugError> {
        if let Some(read_from_log) = self.buffer_file_handle() {
            let mut tracing_log_handle = BufReader::new(read_from_log.try_clone()?);
            std::io::Seek::seek(&mut tracing_log_handle, self.seek_pointer)?;
            let mut buffer_lines = tracing_log_handle.lines();
    ...
    pub(crate) fn setup_logging(
        ...
        Ok(match log_file {
            ...
            None => {
                ...
                // If no log file desitination is specified, we log based on the RUST_LOG setting.
                // The `Option<Layer>` can be used interchangeably with the `Layer` type.
                let buffer_layer = if self.buffer_file.is_some() {
                    Some(
                    ...
                    return Err(DebuggerError::UserMessage(format!(
                        "{}{}",
                        "Unexpected error in creating a buffer file for log messages. ",
