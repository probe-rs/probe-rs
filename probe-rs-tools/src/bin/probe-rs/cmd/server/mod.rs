...
use crate::{Cli, Config, FormatOptions, Subcommand};
...
impl ClientConnection {
    /// Sends a request to the server to run a probe-rs command.
    pub async fn run_command(&mut self, mut cli: Cli) -> anyhow::Result<()> {
        match cli.subcommand {
            ...
            | Subcommand::Trace(_)
            | Subcommand::Itm(_)
            | Subcommand::Info(_) => self.send_command(cli).await,

            // Commands that need a file to be uploaded
            Subcommand::Verify(ref mut cmd) => {
                cmd.path = self.upload_file(&cmd.path).await?;

                self.upload_format_specific_files(&mut cmd.format_options)
                    .await?;

                self.send_command(cli).await
            }
            Subcommand::Attach(crate::cmd::attach::Cmd { run: ref mut cmd })
            | Subcommand::Run(ref mut cmd) => {
                cmd.shared_options.path = self.upload_file(&cmd.shared_options.path).await?;

                self.upload_format_specific_files(&mut cmd.shared_options.format_options)
                    .await?;
    ...
    async fn upload_format_specific_files(
        ...
        if let Some(ref mut idf_bootloader) = format_options.idf_options.idf_bootloader {
            *idf_bootloader = self.upload_file(&idf_bootloader).await?;
        }
        if let Some(ref mut idf_partition_table) = format_options.idf_options.idf_partition_table {
            *idf_partition_table = self.upload_file(&idf_partition_table).await?;
        }

        Ok(())
    }

    async fn send_command(&mut self, mut cli: Cli) -> anyhow::Result<()> {
