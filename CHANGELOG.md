...
- Added chip definition file for Renesas RA4L1, updated algorithm for RA6M5. (#3929) by @inferiorhumanorgans
- `cargo-embed` now searches for its configuration file at the path specified by `PROBE_RS_EMBED_CONFIG_FILE`. (#4003) by @jr1221
- Added support for the Silicon Labs EFR32MG26 Mighty Gecko series (Cortex-M33): generated from SiliconLabs.GeckoPlatform_EFR32MG26_DFP 2025.12.1. (#4001) by @andresv
- Added support for PSOC Control C3 (M3/M5/P2/P5) devices. (#4138) by @Liraelle
- Added `probe-rs mi info` command to print the chip name as JSON. (#3990) by @bugadani
- Added support for writing core registers through the DAP Registers scope. (#4046) by @HalfSweet
- Added support for Infineon PSOC C3 x7/x8 (P7/P8/M7/M8) devices. (#4140) by @Liraelle
- Added support for NXP MCXA5xx family. (#3876) by @felipebalbi
- Added `rtt write <channel_id> <data>` to the debugger REPL. (#3853) by @bugadani
- Added shell completion installation support for fish shell (#3824) by @Be-ing
- `probe-rs profile callstack` can be used to sample call stacks from the device. These profiles can be loaded with `samply` and viewed in firefox profiler. (#3789) by @KingCol13
- Extended `probe-rs profile callstack` to allow using DWARF information to recover callstacks rather than depending on frame pointers. (#3895) by @KingCol13
- Added support for the GD32F4xx series (GD32F403/405/407/425/427/450/470), verified on GD32F425RG hardware. (#3883) by @RationalAsh
- Added stdin/stdout transport for the DAP server. When `--port` is omitted, the server communicates over stdin/stdout instead of TCP, enabling use with DAP clients that launch the debug adapter as a child process (e.g. CLion 2025.3). (#3926) by @Janrupf
- Added chip detection information for ESP32-C61 revision v1.0 (#4051) by @bugadani
- Added support for PSOC Edge E84 devices (#3871) by @jnkr-ifx
- Added Nuclei vendor support module with debug sequences for Nuclei RISC-V chips, including program buffer memory access and NUSPI XIP flash controller handling. (#3893) by @MDr164
- Added support for MCXA185, MCXA186, MCXA255, and MCXA256. (#3890) by @felipebalbi
- Added support for controlling the SWCLK/TCK and SWDIO/TMS pins of J-Link probes via `swj_pins`, which is required for pin-level debug sequences such as the Infineon TLE BootROM debug-mode entry. (#4062) by @mtnmbr
- Added support for the GigaDevice GD32E23x series (Cortex-M23): GD32E230 C/F/G/K variants with 16/32/64 KB flash and GD32E231C4/C6/C8, generated from GigaDevice.GD32E23x_DFP 1.0.2. (#3993) by @aleksander
- Added support for NXP MCXN236 (#4019) by @kalkyl
- Added `rtt::find_rtt_control_block_in_elf` function inside library. (#3786) by @robamu
- Added ADIv6 support for Black Magic Debug probes. (#3903) by @agrif
- Added `--scan-chain` flag to `probe-rs info` command for manual JTAG scan chain override. (#3909) by @MDr164
- On Linux, probes that are detected but can't be opened due to missing permissions (e.g. no udev rule) are now listed and flagged as inaccessible instead of silently disappearing, and the setup hint only fires when it's actually relevant. (#4128) by @Yatekii
- Added faster flash algorithms for Zynq-7000 that run QSPI at higher frequency (#3790) by @adamgreig
- Added support for ESP32-S31 (#4094) by @playfulFence
- probe-rs now recognises ESP-PROG-2 probes. (#3937) by @bugadani
- Added support for flashing and running from RAM for the Zynq7000 (#3772) by @robamu
- Added support for "remote"/"serve" function over Unix sockets. (#3875) by @Gerharddc
- Added `wreg name value` REPL command to set the value of a core or peripheral register. (#4106) by @Hoohaha
- Added/regenerated CH32 target yamls from ch32-metapac, covering CH32F1, CH32H4, CH32L1, CH32V003/CH641, CH32V00X, CH32V1, CH32V2/V3, and CH32X0/CH643. (#4056) by @aq1018
- Added new option `-c/--command` in `probe-rs debug`, allow to execute debug commands from CLI. (#4110) by @Hoohaha
- Added syntax for examining memory pointed to by a register, e.g. `x/8i $lr` (#3959) by @BryanKadzban
- Added an `nRF5340_xxAA_APPONLY` target for application-core-only workflows that must not access or erase the network core. (#4111) by @Vach23
- Added RTT messages to the debug CLI, and a `--no-rtt` flag to disable reading RTT data. (#3851) by @bugadani
- Added support for 64-bit RISC-V cores (RV64). Target YAML files can now declare cores with `type: riscv64`. (#3910) by @MDr164
- Enabled GDB support for Xtensa targets. (#3948) by @bugadani
- Added a function `JtagAccess::set_expected_scan_chain()` to set the expected scan chain. (#3780) by @xobs
- Added a debug sequence that drives the Infineon MOTIX™ TLE98xx/TLE99xx BootROM into debug mode so probe-rs can attach to and flash these devices. (#4062) by @mtnmbr
- Added probe-based chip auto-detection for CH32 targets using the WCH-Link probe's `AttachChip` response, so `probe-rs` can pick the right CH32 variant without a `--chip` flag. (#4057) by @aq1018
- Added capacity to cycle usb power on linux. (#3938) by @xoviat
- Added support for RISC-V Debug Specification Version 1.0. (#4039) by @bugadani
- Added support for some of the NXP S32K3 series (#4060) by @kscz
- Added `RamRegion::is_alias` field to specify whether the RAM region is an alias of another RAM region. (#3800) by @bugadani
- Added support for running `probe-rs dap-server` on a different host from the VSCode client. (#3985) by @noppej
- Added `file:line[:column]` as a breakpoint location format option to the debug console. (#4024) by @bugadani
- Backtraces are now colorized both in CLI and in the debugger REPL, if the client supports ANSI styles. Set `PROBE_RS_COLOR` to a falsey value to disable. (#3951) by @bugadani
- Added autodetection logic for many Renesas chips. (#3752) by @inferiorhumanorgans
- Added target support for the Infineon TLE987x and TLE988x/TLE989x motor controller families. (#4062) by @mtnmbr
- Added initial support for attaching to RP235x RISC-V cores (#3952) by @9names
- Added support for ESP32-P4 v3.1 and 3.2 (#4077) by @bugadani
- Added probe-rs meta package to allow specifying metadata in elf and other binaries. (#3984) by @xoviat
- Added support for the Microchip/Atmel ATSAMG family, verified on ATSAMG55J19 hardware. (#3923) by @he2she
- Added `force_scan_chain` field to JTAG configuration in target YAML files. When set to true, bypasses JTAG auto-detection and uses the specified scan chain directly. (#3909) by @MDr164
- Added flash algorithm for the STM32N6570-DK (MX66UW1G45G external NOR flash on XSPI2) and fixed the XSPI2NOR memory region size. (#3849) by @tarfu
- `probe-rs run` and `probe-rs attach` can now send data to RTT channels. If the program detects down channels, it will display a prompt. Use `ctrl-d` to cycle between channels. (#3746) by @bugadani
- Cortex-R5F (AM2612) support added to library and CLI with semihosting and vector catch<br> (#3844) by @korken89
- Added `probe-rs-linux` plugin crate with two SWD probe drivers: `linuxgpiod` (GPIO bit-bang) and `linuxspidevswd` (spidev with PICO/POCI tied through a resistor). (#3986) by @kraln
- Added an ARM debug sequence for Silicon Labs EFM32 Series 1 Geckos (EFM32PG1/JG1, EFR32xG1x) that catches reset via a hardware breakpoint on the reset vector, so probe-rs can reliably halt the core out of reset and attach to these devices. (#4075) by @dsludwig
- Added SiFive FU740-C000 (HiFive Unmatched) support with debug sequences for the e51 monitor core, including ebreakm bootstrap, program buffer memory access, and QSPI0 XIP flash controller handling. (#3958) by @MDr164
- target-gen now automatically extracts ELF symbols prefixed with `VendorFunc_` as vendor-specific flash algorithm entry points. (#3989) by @bugadani

### Changed

- `FlashError::RoutineCallFailed` field `name` changed from `&'static str` to `String`. Added `FlashError::VendorFunctionMissing` for calls to undefined vendor functions. (#3989) by @bugadani
- Changed `SwdSequence` to `pub` to allow for implementing `ArmDebugSequence` externally. (#3864) by @xobs
- * The `coredump` feature in the `probe-rs` crate can now be used to disable support for writing core dumps, which reduces the number of dependencies if disabled. (#3773) by @Tiwalun
- Changed the ICEPICK initialization sequence to print out a diagnostic scan (#3780) by @xobs
- Improved T32CZ20 flash algorithm: ~10x speed increase by writing multiple pages per iteration and correcting the flash end address to account for reserved pages. (#4020) by @andersesbensen
- Changed MCXA5xx Flash algorithm page size to 4096 bytes (#3943) by @felipebalbi
- ˙`RiscvCommunicationInterface::halt` now correctly re-enables ebreak-in-debug even if all cores halted. (#4095) by @bugadani
- `probe-rs debug` now takes the binary as a positional argument. `--exe` has been removed, and `--launch`, `--verify` and `--preverify` have been added. (#3852) by @bugadani
- `probe-rs read` now supports multiple output formats when printing to console. The default output format has been changed to `hex-table`. (#3847) by @bugadani
- Espressif device support for probe-rs has been moved to the `probe-rs-espressif` crate. (#3813) by @bugadani
- * The `builtin-formats` feature in the `probe-rs` crate can now be used to disable support for built-in firmware image formats (bin, hex, elf, uf2), which reduces the number of dependencies if disabled. (#3817) by @bugadani
- Moved `DebugprobeSelector` into its own module, remove some conversion traits to focus on `FromStr` implementation. Updated documentation to mention possibility to select USB interface. (#4142) by @Yatekii
- The GDB server now tolerates cores that firmware has not enabled yet (e.g. a secondary core still held in reset): such cores are skipped when halting and reporting stop reasons, and accesses to them return `EIO` instead of aborting the session, so debugging the remaining cores keeps working. (#4134) by @Liraelle
- The `info reg` REPL command now shows a list of registers that could not be read, along with their error messages. (#4096) by @bugadani
- Replaced the hacked-in ESP32-specific `read_flash_size` flash algo function with support for arbitrary vendor-specific functions. (#3989) by @bugadani
- `probe-rs info` command now attempts to autodetect the target and print the chip name. The old behavior has been renamed to `probe-rs info --verbose`. (#3977) by @jnkr-ifx
- Improved the PSOC Edge E84 debug sequence reliability, extended list of supported targets and their memory map layout. (#4134) by @Liraelle
- Changed EFM32PG1B Series flash algorithm page size to 8192 bytes to speed up flashing. (#4074) by @dsludwig
- `RawFlashAlgorithm` no longer implements `Hash`. (#3989) by @bugadani
- Armv7A banked register operations now use optimized AP operations for consecutive transfers,
  which can improve performance for larger memory operations significantly. (#4142) by @Yatekii
- FTDI probes now respect the interface number from `--probe VID:PID-INTERFACE` to select the FTDI channel (0=A, 1=B, 2=C, 3=D). Previously all FTDI probes always used Channel A. (#3908) by @MDr164
- Updated the CMSIS-DAP v2 USB endpoint handling to retain a persistent endpoint handle. (#4099) by @teburd
- The `stack_trace` RPC call now returns a structured response instead of a plain string. (#3936) by @bugadani
- `probe-rs info` now prints a hint that a failed or incomplete result does not necessarily mean the chip or wiring is broken, since flashing and debugging may still work. (#4117) by @Yatekii
- The ActiveFlasher and some other, previously private flashing related types have been marked as public (#3817) by @bugadani
- `probe-rs debug` no longer requires a firmware. (#3859) by @bugadani
- Disassembly no longer requires debug symbols (#3959) by @BryanKadzban
- Updated the STM32N6 MX66UW1G45G flash algorithm and changed its page and sector size to 4096 bytes to better support multi-partition workflows. (#3988) by @tarfu
- `probe-rs attach` no longer requires a firmware path.
  `probe-rs run` and `attach`: the `--rtt-scan-memory` option has been replaced by `--scan-region` which accepts an address or a range. (#3747) by @bugadani
- The ARM AP registers (`CSW`, `IDR`, `BASE`, ...) generated by `define_ap_register!` are now `bitfield!` newtypes over the raw `u32` instead of plain structs with public fields. Field access moves from `reg.Field` / `reg.Field = x` to the `reg.Field()` / `reg.set_Field(x)` accessors. Enum-backed fields keep infallible getters: an encoding that isn't a known variant is returned as the enum's new `Unknown(bits)` variant rather than failing to parse. This removes the hand-written, duplicated bit-packing in each register's `from`/`to` closures. (#4125) by @Yatekii
- The `vendor::register_vendor` function has been replaced with `plugin::register_plugin` and the `Plugin` struct. The `Plugin` struct can be used to extend the list of built-in targets, image formats, probe drivers and vendor support code. (#3834) by @bugadani
- Increased ESP32-S3 max CPU speed during flashing (#3953) by @bugadani
- Changed the RP2350 to run through the bootrom sequence to increase programming speed (#3898) by @xobs
- Changed `JtagAccess::set_scan_chain()` to set the scan chain, rather than the expected scan chain. (#3780) by @xobs
- Updated the DAP `debugProtocol.json` schema to the upstream revision dated 2026-02-06. (#3969) by @github-actions[bot]
- Changed the DAP server to emit DWARF source paths verbatim in stack frames, breakpoints, and disassembly. (#3985) by @noppej
- Changed MCXA27x Flash algorithm page size to 4096 bytes (#3785) by @felipebalbi
- Increased page program timeout for ESP32 devices from 1 to 2 seconds (#3922) by @bugadani
- Updated autodetection logic for Renesas RA chips to CMSIS Pack 6.4.0. (#4004) by @inferiorhumanorgans
- Changed MCXA26x and MCXA25x Flash algorithm page size to 4096 bytes (#3944) by @felipebalbi
- `serde_yaml` has been replaced with `yaml_serde` (#3998) by @bugadani

### Fixed

- Downgrade gdbstub to version 0.7.8 to avoid MultiThread regression #3805 (#4021) by @marc-hb
- Selecting a chip that uses APv2 addresses (e.g. RP2350) while a different, DPv1-only target is connected now returns a proper error instead of panicking with "entered unreachable code". (#4129) by @Yatekii
- Fixed hardware breakpoints being silently ignored after a chip reset. The cached `hw_breakpoints_enabled` state was not invalidated on reset, causing the breakpoint unit to remain globally disabled even though breakpoint comparator registers were programmed correctly. (#3810) by @fmahon
- Fixed `probe-rs info` aborting with a raw probe/transport error (e.g. `Failed to send the JTAG sequence to the probe`, or `interface Jtag must be selected for this operation`) during chip auto-detection on SWD-only boards. RISC-V/Xtensa auto-detection errors are now treated as "chip not found for this architecture" instead of aborting the whole auto-detection sequence. (#4151) by @Liraelle
- dap-server: SVD registers without an `<access>` tag are now readable in the debugger instead of being marked restricted. This makes peripheral registers viewable for chips whose SVDs omit `<access>` (e.g. STM32H5). (#4123) by @Yatekii
- Fixed the reported transfer width when an APB4/APB5 memory AP rejects a non-32-bit access: it used the `DataSize` discriminant instead of the byte count, so e.g. a 64-bit access was reported as "24 bits" instead of 64. (#4126) by @Yatekii
- * cmsisdap: Check buffer size of response when executing a JTAG sequence. (#3833) by @Tiwalun
- RISC-V: Fixed a bug in the communication interface, potentially causing arbitrary data being read and writes not taking effect. (#3870) by @hdbhm
- Fixed duplicates detection of devices with empty serial number when using cmsis-dap. (#3907) by @bastien-buil
- cmsisdap: Handle faults in block transfers the same way as faults in single transfers. (#3806) by @Tiwalun
- Reset the JTAG TAP before reading IDCODE in both RISC-V and Xtensa to handle indeterminate state after attach, fixing `probe-rs info` returning no chip. (#3914) by @MDr164
- Xtensa: Fixed a bug where instruction stepping trashed a CPU register. (#3803) by @bugadani
- Fixed RTT control block clearing in the DAP server to happen eagerly before reset (while the core is halted) instead of being skipped when `haltAfterReset` is `false`. The previous workaround disabled clearing entirely, which could lead to attaching to stale RTT data from a previous session. (#3925) by @Janrupf
- Fixed reset on the TMS570, where the ICEPICK reset wasn't actually performing a reset. (#3780) by @xobs
- Reading the debug port when no target is connected (or the target is unpowered) now returns a clear "no target responded" error that points at connection/power/wiring, instead of a cryptic SWD protocol error. (#4127) by @Yatekii
- Fixed automatic target detection aborting on JTAG-capable probes that are using SWD. (#4114) by @Vach23
- Fixed `probe-rs profile --reset` aborting when a disabled core was encountered; disabled cores are now skipped instead of returning an error. (#4156) by @Liraelle
- Fixed unwinding Xtensa stack frames when unwind info is available. (#3972) by @bugadani
- Fixed `profile callstack` failing to compute a debug ID for ELF files whose code section is named `text` instead of `.text`. (#4154) by @Liraelle
- Fixed bogus source locations and `<unknown function>` names in backtraces for inlined functions whose abstract origin lives in another compilation unit. The `DW_AT_specification` of such a cross-unit abstract origin was resolved against the wrong unit, landing on an unrelated DIE and producing nonsensical line/column numbers. (#4086) by @Dirbaio
- Fixed error when using `probe-rs run` with a non-ELF binary (#3949) by @jnkr-ifx
- cmsisdap: a read that comes back without data now returns an error instead of panicking, which could happen during session cleanup. (#4130) by @Yatekii
- Fixed the GDB stub sometimes reporting a spurious `SIGINT` right after `continue`, halting the debugger one instruction past the breakpoint instead of running to the next one. The resume path now waits for the core to actually leave the halted state before polling for the next stop. (#4136) by @Yatekii
- Fixed a bug in Zynq-7000 flash algorithm that broke loads beyond 64kB (#3790) by @adamgreig
- Fixed flashing ESP32-S3 devices with octal SPI flash chips (#3825) by @bugadani
- Fixed spelling across the entire repository in comments and in code (#3781) by @xobs
- Fixed NSRST for Glasgow probe (#4073) by @craigjb
- Fixed the GDB `load` command failing immediately with "Load failed" on targets whose flash does not start at address 0. The GDB memory map advertised flash regions using the flash algorithm's region-relative sector offset instead of the absolute address, so `vFlashErase`/`vFlashWrite` targeted the wrong addresses. The region base is now added back to the advertised flash start. (#4152) by @Liraelle
- Fixed flashing for EFM32/EFR32 Gecko S2C3 series. (#3845) by @jhlywa
- Fixed RISC-V program-buffer execution on strict debug modules by setting an XLEN-appropriate `aarsize` on postexec-only abstract commands. (#4103) by @sanchuanhehe
- cortex-m: keep state during single-step to disambiguate halt reason (#4040) by @9names
- Fixed `CH32H4_Series` SYS region marked as read-only; aligned with other CH32 families and added the `ch32-h4-sys` flash algorithm. (#4058) by @aq1018
- Fixed STM32N6 register reset sequence to use the correct ARMv8-M register set, ensuring security extension registers are properly zeroed after the boot ROM runs. (#3843) by @tarfu
- Fixed reset on TMS570 due to ITR sometimes getting disabled (#3808) by @xobs
- Fixed RTT control block corruption when `embedded-test` resets the device between tests. (#3899) by @ChristianJacobsen
- RISC-V: Fixed debug module cleanup to prevent the DM from getting stuck after abrupt disconnects. (#3915) by @MDr164
- `probe-rs attach` and `probe-rs dap-server` are fixed to send a missing newline character `\n` when they write down a line to an RTT channel. (#3901) by @akscram
- Fixed `truncate` flag in semihosting, which was incorrectly set for cases that didn't require write access.  (#3960) by @4Kamei
- Fixed a panic in the `probe-rs debug` CLI when its internal event channel filled up under heavy RTT or output load. (#4155) by @Liraelle
- Handle spurious NACKs from Ctrl/Stat reads during power up (#3976) by @jnkr-ifx
- Fixed the EFM32PG26 chip name not matching the Series-2 Configuration 3 debug-sequence detection in `efm32xg2.rs` (was typoed as `EFR32PG26`, which is not a Silicon Labs product). EFM32PG26 now correctly uses the `0x0800_0000` flash base and MSC mass-erase path introduced in #3845. (#4001) by @andresv
- Fix a panic which could happen when trying to select a probe without specifying a serial number. (#3797) by @Tiwalun
- arm: When a lockup is detected, don't stop the debugger. (#3799) by @Tiwalun
- Fixed MIMXRT1015 flashing: disable the MPU left enabled by the boot ROM after reset catch. (#4109) by @Hoohaha
- Stopped logging a spurious "Failed to switch to DP" warning on multidrop targets (e.g. RP2040) when the quick DP connect fails but the full setup sequence succeeds. The failed quick connect is now logged at debug level, and a genuine failure is still surfaced as an error. (#4133) by @Yatekii
- Fixed probe-rs not being able to interact with MCXA2xx after a full FLASH erase. (#3892) by @Wassasin
- Handle DPv3 target descriptions that use legacy AP index 0 as APv2 base address 0x0. (#4032) by @ref42
- Fix problems flashing MCXN947 (#3997) by @wjeffreys96
- Fixed flashing >128KB binaries on 256KB STM32G47x devices. (#3916) by @Rechenmaschine
- Fixed reset sequence for ESP32-C5, ESP32-C61 and ESP32-H2 devices. (#3848) by @bugadani
- The GDB server deduplicates flash regions with the same address but different block sizes, allowing GDB to function on targets with flash supporting multiple block sizes. (#3978) by @jnkr-ifx
- Fixed flashing newer ESP32-H2 revisions. (#3842) by @bugadani
- STM32WBA65RI can now be auto-detected without specifying `--chip` (part number 0x4b0 sourced from hardware). STM32WBA chips are also now correctly assigned the ARMv8-M debug sequence instead of ARMv7. (#3889) by @leftger
- The MIMXRT11xx debug sequence supports another boot ROM spin address, for
  revision B silicon. (#4017) by @mciantyre

### Removed

- Removed the legacy `CH32V0_Series.yaml`, `CH32V2_Series.yaml`, and `CH32V3_Series.yaml` (superseded by ch32-metapac-generated family yamls). (#4056) by @aq1018
- Removed `pc_flash_size` from flash algorithm target descriptions. Use `vendor_functions: {FlashSize: 0x...}` instead. (#3989) by @bugadani
- rtthost has been discontinued in favour of `probe-rs attach` and `probe-rs run`. (#3802) by @bugadani
- `read` method of ADI memory interface could read outside of the address range for unaligned
  start addresses/blocks. Removed so that default trait implementation is used instead. (#3932) by @robamu
...
[unreleased]: https://github.com/probe-rs/probe-rs/compare/v0.32.0...master
[0.32.0]: https://github.com/probe-rs/probe-rs/compare/v0.31.0...v0.32.0
