...
### Added
 ...
 - Ability to check chip/firmware compatibility for ESP chips. (#3760) by @Yatekii
 - Added `rtt_poll_interval` to flash algorithms. Defaults to 20ms. (#3760) by @Yatekii
 - Basic Zynq-7000 SoC support. QSPI flashing and RTT attaching supported. (#3760) by @Yatekii
 - Added the `--prefer-flash-algorithm` CLI option which allows overriding the flash algorithms to use
   for different memory regions. (#3760) by @Yatekii
 - Add support for ch32f1 (#3760) by @Yatekii
 - Added support for the PIC32CXSG series of chips (#3760) by @Yatekii
 - Add support for ESP32-C5 (#3760) by @Yatekii
 - Added `MemoryInterface::execute_memory_operations` and associated types to support executing multiple memory operations in a single batch, which is useful for architectures that don't support background memory access. (#3760) by @Yatekii
 - Added the `test` command to the debug console, when running embedded-test firmware. (#3760) by @Yatekii
 - Add support for ESP32-P4 (#3760) by @Yatekii
 - Add support for ESP32-C61 (#3760) by @Yatekii
 - Added `CoreInterface::spill_registers` method. (#3760) by @Yatekii
 - Check presence of app descriptor required by ESP-IDF bootloader (#3760) by @Yatekii
 - `cargo-flash` can now be compiled with the `remote` feature to support remote flashing. This feature enables the `--host` and `--token` flags to specify the remote server and authentication token. (#3760) by @Yatekii
 - Added `--read-flasher-rtt` CLI switch to enable reading RTT output from the flash loader.
   ...
   Added `read_flasher_rtt` parameter to `probe_rs::flashing::{erase, erase_all, run_blank_check}`. (#3760) by @Yatekii
 - Add ROM regions to the ESP32 memory maps. (#3760) by @Yatekii
 - `cargo-flash`/`cargo-embed`: Added support for CLI presets via the probe-rs config file and the `--preset` flag. (#3760) by @Yatekii
 - Added support for the STM32WB0 series. (#3760) by @Yatekii
 - Added support the STM32U3 family of processors. (#3760) by @Yatekii
 - Add target support for MSPM0G511x and MSPM0518x (#3760) by @Yatekii
 - Changed preset logic to fallback to "default" preset if present (#3760) by @Yatekii
 - Added support for probes with multiple interfaces (#3760) by @Yatekii
 - Added `--idf-flash-freq` and `--idf-flash-mode` options. (#3760) by @Yatekii
 - Add support for cw32l0 (#3760) by @Yatekii
 - Add support for CH32V003 RV32EC RISC-V MCU (#3760) by @Yatekii
 - add Holtek vendor support for HT32F523x2 series (#3760) by @Yatekii
 - Add support for the nRF54LM20A<br> (#3760) by @Yatekii
...
### Changed
 ...
 - Updated target for STM32C0 family with the new flashing algorithms in version 2.4.0 of STM32C0xx_DFP.  This fixes the SwdDpWait warning during the erase phase (#3505).
   Also added support for the STM32C051 and STM32C091. (#3760) by @Yatekii
 - ESP32-specific flash size detection has been reimplemented as a custom flash algorithm function. (#3760) by @Yatekii
 - Function names now include the type name. (#3760) by @Yatekii
 - Changed the warning macro that is emitted when a CIDR value doesn't match to an info block since there's nothing probe-rs or the user can do about it. (#3760) by @Yatekii
 - Change reset sequence for ESP32 Xtensa devices to improve reliability. (#3760) by @Yatekii
 - The flashing code now uses a A32-encoded BKPT instruction for ARMv7A targets. (#3760) by @Yatekii
 - Updated nusb dependency to version 0.2.0 (#3760) by @Yatekii
 - Update LPC55S16 Target (#3760) by @Yatekii
 - Change several RiscvCommunicationsInterface functions so that they are now `pub` rather than `pub(crate)`, bringing them in-line with the Xtensa architecture (#3760) by @Yatekii
 - Changed ESP32-specific Xtensa and RISC-V `detect_flash_size` debug sequence function to take a `Core` reference instead of a `Session` reference. (#3760) by @Yatekii

### Fixed

 - Fixed an underflow in Xtensa register unwinding. (#3760) by @Yatekii
 - Fixed attaching to RISC-V cores when one or more hart is not enabled. (#3760) by @Yatekii
 - Fixed some Xtensa performance issues. (#3760) by @Yatekii
 - Fixed support for onboard Digilent HS1 probes (#3760) by @Yatekii
 - Bug fixes for ARMv7A debug:
   ...
   - Use reset vector catch for reset-halt instead of requesting halt then
     resetting
   - Warn if core isn't halted after reset-halt (#3760) by @Yatekii
 - Fixed CH347 opening logic to account for both CH347T and CH347F (#3760) by @Yatekii
 - Fixed RISC-V tunneled JTAG transport for some probes (including Digilent HS1) (#3760) by @Yatekii
 - Fixed the RP235x reset sequence by giving it a timeout rather than a loop limit (#3760) by @Yatekii
 - Add banked stack pointer registers for v8m targets, including TrustZone and non-TrustZone (#3760) by @Yatekii
 - `preverify` is no longer ignored in cargo-embed. (#3760) by @Yatekii
 - Change CMSIS-DAP probe maximum packet size detection to speed up connecting to EDBG probes (#3760) by @Yatekii
 - Fixed polling long-running Xtensa instructions. (#3760) by @Yatekii
 - CMSIS-DAP now passes the JTAG TAP index from the JTAG chain to DAP_Transfer and DAP_TransferBlock requests sent to the probe. (#3760) by @Yatekii
 - Fixed RTT on 64-bit targets so all flag and offset fields are 32-bits (#3760) by @Yatekii
 - Fixed flashing of ConfigROM and DataFlash regions for Renesas RA series targets by correcting memory map addresses and region types. (#3760) by @Yatekii
 - Don't panic if the USB configurations for a ST-Link are not accessible, but return an error instead. (#3760) by @Yatekii
 - When enabling data tracing in a DWT unit, access matches on word size, not byte size. (#3760) by @Yatekii
 - Revise the MIMXRT10xx reset approach to better support RAM loading. (#3760) by @Yatekii
 - ESP32-S3: fixed flashing empty devices, added code to support octal flash. (#3760) by @Yatekii
 - Fixed incorrect group checks for the current user in linux help error message during setup. (#3760) by @Yatekii
 - Clear DP cache when disconnecting from an ARM communication interface (#3760) by @Yatekii
 - Fixed ATSAMV71 flash corruption by offsetting the flash algorithm instructions,
   see target file for details. (#3760) by @Yatekii
 - Fixed memory inspectors crashing when reading memory with a negative offset. (#3760) by @Yatekii
 - Fixed nRF53 debug unlock failing after mass erase due to checking the wrong AP register. (#3760) by @Yatekii
 - Fix DAP watch evaluations for static variables by loading only requested globals lazily while preserving fast value updates between steps. (#3760) by @Yatekii
 - Fixed a bug which caused some RISC-V processors to be halted unnecessarily. (#3760) by @Yatekii

### Removed

 - Removed `CoreInterface::on_session_stop` (#3760) by @Yatekii
 - Removed `{Core, CoreInterface}::debug_on_sw_breakpoint`. Architecture implementations should enable software breakpoints on connection. (#3760) by @Yatekii
