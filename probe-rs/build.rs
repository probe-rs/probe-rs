...
impl CrcBinaryValidator {
    ...
    pub fn validate_binary_size(&self, size: u64) -> Result<(), String> {
        if size < self.min_size {
            Err(format!(
                "Binary too small: {} bytes (minimum: {} bytes)",
                size, self.min_size
            ))
        } else if size > self.max_size {
            Err(format!(
                "Binary too large: {} bytes (maximum: {} bytes)",
                size, self.max_size
            ))
        } else {
            Ok(())
    ...
    pub fn validate_metadata_consistency(
        ...
        if metadata_size != actual_size {
            Err(format!(
                "Size mismatch: metadata says {metadata_size} bytes, actual binary is {actual_size} bytes"
            ))
        } else {
            Ok(())
        }
...
fn check_crc_binaries() {
    ...
    for (target, description) in ARM_TARGETS {
        let bin_path = format!("../crc32_algorithms/{target}.bin");
        let toml_path = format!("../crc32_algorithms/{target}.toml");
        ...
        if !PathBuf::from(&toml_path).exists() {
            missing_files.push(format!("{toml_path} (metadata for {description})"));
        }
    }
...
mod tests {
    ...
    fn test_known_firmware_sizes() {
        ...
        assert!(
            validator.validate_binary_size(1144).is_ok(),
            "Optimized firmware size should be valid"
        );
        assert!(
            validator.validate_binary_size(96).is_ok(),
            "Simple firmware size should be valid"
        );
        assert!(
            validator.validate_binary_size(188).is_ok(),
            "Simple word firmware size should be valid"
        );

        // Common problematic sizes should fail
        assert!(
            validator.validate_binary_size(66_000).is_err(),
            "Large objcopy artifact should fail"
        );
        assert!(
            validator.validate_binary_size(10).is_err(),
            "Tiny binary should fail"
        );
    }
