...
//! between `read_word_32` in a loop and `ArmMemoryInterface::execute_operations`.

use anyhow::{Context, Result};
use clap::Parser;
use probe_rs::architecture::arm::FullyQualifiedApAddress;
use probe_rs::probe::{WireProtocol, list::Lister};
use probe_rs::{MemoryInterface, Operation, OperationKind, Permissions, config::TargetSelector};
...
fn main() -> Result<()> {
    ...
    // `execute_operations` is on `ArmMemoryInterface`, below the architecture-agnostic `Core`, so
    ...
    let mut operations: Vec<Operation<'_>> = addresses
        .iter()
        .zip(batched.iter_mut())
        .map(|(&address, value)| {
            Operation::new(address, OperationKind::Read32(std::slice::from_mut(value)))
        })
        .collect();
    let start = Instant::now();
    memory.execute_operations(&mut operations)?;
