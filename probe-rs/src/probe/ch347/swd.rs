//! The SWD engine.
//!
//! The firmware runs a whole batch and samples the ACKs itself, so WAIT is handled here.
//! WAITed accesses are retried with a backoff, and a WAIT with a write behind it is
//! reported as is so nothing runs twice. After a WAIT it gives up on, and after a FAULT,
//! the firmware still clocks a data phase, which leaves the port in protocol error, so
//! every WAIT or FAULT is followed by a line reset. Overrun detection stays off.
...
/// Clocks with SWDIO high that reset the wire protocol, and the idle clocks that end the reset.
const LINE_RESET_HIGH: usize = 51;
const LINE_RESET_IDLE: usize = 3;
...
/// Retries after a WAIT, with a pause doubling from [`WAIT_BACKOFF`] to 64 times it:
/// about 110 ms in all, enough for an nRF91 page erase.
...
impl Register {
    ...
    /// The same address as ABORT, which a read answers with DPIDR.
    const DPIDR: Self = Self::dp(0x0);
...
impl Ch347Device {
    ...
    /// Puts the SW-DP back in protocol after a WAIT or FAULT reply, best effort.
    fn resync(&mut self) {
        match self.line_reset() {
            Ok(Some(dpidr)) => tracing::debug!("SW-DP resynced, DPIDR {dpidr:#010x}"),
            Ok(None) => tracing::debug!("SW-DP resync not confirmed"),
            Err(error) => tracing::debug!("SW-DP resync failed: {error}"),
        }
    }

    /// A line reset followed by the DPIDR read, which answers with DPIDR if the port is back.
    fn line_reset(&mut self) -> Result<Option<u32>, DebugProbeError> {
        self.line_sequence(&BitSequence::repeat(true, LINE_RESET_HIGH))?;
        self.line_sequence(&BitSequence::repeat(false, LINE_RESET_IDLE))?;
        let mut batch = self.batch(None);
        batch.push(Op::read(Register::DPIDR))?;
        Ok(match self.execute(&batch)?.first() {
            Some(Ok(value)) => *value,
            _ => None,
        })
    }
...
mod tests {
    ...
    /// The line reset and DPIDR read the driver sends after every WAIT or FAULT.
    fn resync() -> Vec<(Vec<u8>, Vec<u8>)> {
        vec![
            (
                frame(&[0xA1, 51, 0, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x07]),
                frame(&[SUB_SEQUENCE]),
            ),
            (frame(&[0xA1, 3, 0, 0]), frame(&[SUB_SEQUENCE])),
            (req(&[Op::read(DPIDR)]), reply(&[ok(0x6BA0_2477)])),
        ]
    }

    /// Owned frames as the slices `device` takes.
    fn borrowed(frames: &[(Vec<u8>, Vec<u8>)]) -> Vec<(&[u8], &[u8])> {
        frames
            .iter()
    ...
    fn a_write_wait_is_retried_with_backoff_then_gives_up() {
        ...
        // once the port is resynced.
        let mut frames = vec![(request.clone(), reply(&[wrote(ACK_WAIT)]))];
        frames.extend(resync());
        frames.push((request.clone(), reply(&[wrote(ACK_OK)])));
        ...
        for _ in 0..=WAIT_RETRIES {
            frames.push((request.clone(), reply(&[wrote(ACK_WAIT)])));
            frames.extend(resync());
        }
        frames.push((frame(DAPABORT), frame(WRITE_OK)));
        let (mut dev, script) = device(&borrowed(&frames));
    ...
    fn a_wait_before_a_further_write_is_not_retried() {
        ...
        let mut frames = vec![(request, reply(&[wrote(ACK_WAIT), wrote(ACK_OK)]))];
        frames.extend(resync());
        frames.push((frame(DAPABORT), frame(WRITE_OK)));
        let (mut dev, script) = device(&borrowed(&frames));
    ...
    fn wait_on_the_rdbuff_read_is_retried() {
        ...
        frames.extend(resync());
        frames.push((req(&[Op::read(RDBUFF)]), reply(&[ok(0x55)])));
        let (mut dev, script) = device(&borrowed(&frames));
    ...
    fn fault_clears_the_sticky_flags() {
        // A FAULT is reported only after the port is resynced, CTRL/STAT is read and ABORT
        // clears the sticky bits.
        let mut frames = vec![(req(&[Op::read(DPIDR)]), reply(&[read_reply(ACK_FAULT, 0)]))];
        frames.extend(resync());
        frames.push((req(&[Op::read(CTRL)]), reply(&[ok(0x20)])));
        frames.push((req(&[Op::write(ABORT, 0x1E)]), frame(WRITE_OK)));
        let (mut dev, script) = device(&borrowed(&frames));
