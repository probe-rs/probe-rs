...
use crate::probe::ti_icdi::{IcdiError, decode_hex};

pub trait GdbRemoteInterface {
    fn get_max_packet_size(&self) -> usize;

    /// Read a core register via the GDB `p` packet.
    #[allow(dead_code)]
    fn read_reg(&mut self, regsel: u32) -> Result<u32, DebugProbeError> {
        ...
        tracing::trace!("read reg response {:?}", x);
        let bytes = decode_hex(x)?;
        let y: [u8; 4] = bytes.try_into().map_err(|_| IcdiError::HexDecode)?;

        Ok(u32::from_le_bytes(y))
    }

    /// Write a core register via the GDB `P` packet.
    #[allow(dead_code)]
    ...
    fn read_mem(&mut self, mut addr: u32, data: &mut [u8]) -> Result<(), DebugProbeError> {
        ...
        for chunk in data.chunks_mut(self.get_max_packet_size() / 2 - 7) {
            ...
            response
                .get_payload()?
                .strip_prefix(b"OK:")
                .ok_or(IcdiError::MissingOkPrefix)?
                .iter()
                .filter_map(|&ch| {
                    if escaped {
                        escaped = false;
            ...
            if byte_cnt != chunk.len() {
                return Err(IcdiError::ShortRead.into());
            }
    ...
    fn write_mem(&mut self, mut addr: u32, data: &[u8]) -> Result<(), DebugProbeError> {
        ...
        while cur.len() > 0 {
            ...
            while buf.len().saturating_sub(5) <= max {
