...
#[derive(Default, Debug, Clone, Eq, Hash, Serialize, Deserialize, postcard_schema::Schema)]
...
impl PartialEq for UsbFilters {
    fn eq(&self, other: &Self) -> bool {
        ...
        let serial_port_matches =
            check_eq(self.serial_number.as_ref(), other.serial_number.as_ref());
        let hid_interface_matches =
            check_eq(self.hid_interface.as_ref(), other.hid_interface.as_ref());
        #[cfg(not(any(target_os = "linux", target_os = "android")))]
        let sysfs_path_matches = true;
        #[cfg(not(target_os = "windows"))]
        let instance_id_matches = true;
        #[cfg(not(target_os = "windows"))]
        let parent_instance_id_matches = true;
        #[cfg(not(target_os = "windows"))]
        let port_number_matches = true;
        #[cfg(not(target_os = "windows"))]
        let driver_matches = true;
        #[cfg(not(target_os = "macos"))]
        let registry_id_matches = true;
        #[cfg(not(target_os = "macos"))]
        let location_id_matches = true;

        #[cfg(any(target_os = "linux", target_os = "android"))]
        let sysfs_path_matches = check_eq(self.sysfs_path.as_ref(), other.sysfs_path.as_ref());

        #[cfg(target_os = "windows")]
        let instance_id_matches = check_eq(self.instance_id.as_ref(), other.instance_id.as_ref());
        #[cfg(target_os = "windows")]
        let parent_instance_id_matches = check_eq(
            self.parent_instance_id.as_ref(),
            other.parent_instance_id.as_ref(),
        );
        #[cfg(target_os = "windows")]
        let port_number_matches = check_eq(self.port_number.as_ref(), other.port_number.as_ref());
        #[cfg(target_os = "windows")]
        let driver_matches = check_eq(self.driver.as_ref(), other.driver.as_ref());

        #[cfg(target_os = "macos")]
        let registry_id_matches = check_eq(self.registry_id.as_ref(), other.registry_id.as_ref());
        #[cfg(target_os = "macos")]
        let location_id_matches = check_eq(self.location_id.as_ref(), other.location_id.as_ref());

        serial_port_matches
            && hid_interface_matches
            && sysfs_path_matches
            && instance_id_matches
            && parent_instance_id_matches
            && port_number_matches
            && driver_matches
            && registry_id_matches
            && location_id_matches
...
trait OptionFilter<T> {
    fn filter_option<U>(&self, val: U) -> bool
    where
        for<'a> &'a T: PartialEq<U>;
    fn filter_optional<U>(&self, val: Option<U>) -> bool
    where
        for<'a> &'a T: PartialEq<U>;
}
...
impl UsbFilters {
    pub(crate) fn matches(&self, info: &DeviceInfo) -> bool {
        let serial_port_matches = self.serial_number.filter_optional(info.serial_number());
        let hid_interface_matches = self
            .hid_interface
            .map(|ifnum| {
                info.interfaces()
        ...
        #[cfg(any(target_os = "linux", target_os = "android"))]
        let sysfs_path_matches = self.sysfs_path.filter_option(info.sysfs_path());

        #[cfg(target_os = "windows")]
        let instance_id_matches = self
            .instance_id
            .as_ref()
            .map(AsRef::<std::ffi::OsStr>::as_ref)
            .filter_option(&info.instance_id());
        #[cfg(target_os = "windows")]
        let parent_instance_id_matches = self
            .parent_instance_id
            .as_ref()
            .map(AsRef::<std::ffi::OsStr>::as_ref)
            .filter_option(&info.parent_instance_id());
        #[cfg(target_os = "windows")]
        let port_number_matches = self.port_number.filter_option(&info.port_number());
        #[cfg(target_os = "windows")]
        let driver_matches = self.driver.filter_optional(info.driver());

        #[cfg(target_os = "macos")]
        let registry_id_matches = self.registry_id.filter_option(&info.registry_id());
        #[cfg(target_os = "macos")]
        let location_id_matches = self.location_id.filter_option(&info.location_id());
...
/// Wrapper for `SocketAddr` that implements `postcard_schema::Schema`.
pub struct SocketAddrWrapper(SocketAddr);

impl<'a> PartialEq<&'a SocketAddrWrapper> for SocketAddrWrapper {
    fn eq(&self, other: &&'a SocketAddrWrapper) -> bool {
        self.eq(*other)
    }
}

impl PartialEq<SocketAddrWrapper> for &SocketAddrWrapper {
    fn eq(&self, other: &SocketAddrWrapper) -> bool {
        (*self).eq(other)
    }
}

impl AsRef<SocketAddr> for SocketAddrWrapper {
    fn as_ref(&self) -> &SocketAddr {
        &self.0
    }
}

impl Deref for SocketAddrWrapper {
...
impl DerefMut for SocketAddrWrapper {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}

impl std::hash::Hash for SocketAddrWrapper {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        self.0.hash(state)
    }
}

impl std::fmt::Display for SocketAddrWrapper {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.deref().fmt(f)
    }
}
...
impl Serialize for SocketAddrWrapper {
    ...
    where
        S: serde::Serializer,
    {
        <SocketAddr as Serialize>::serialize(self.deref(), serializer)
    }
}
...
impl<'de> Deserialize<'de> for SocketAddrWrapper {
    ...
    where
        D: Deserializer<'de>,
    {
        <SocketAddr as Deserialize>::deserialize(deserializer).map(SocketAddrWrapper)
    }
}

impl postcard_schema::Schema for SocketAddrWrapper {
    const SCHEMA: &'static postcard_schema::schema::NamedType = {
        ...
        &NamedType {
            name: "SocketAddrWrapper",
            ty: &DataModelType::NewtypeStruct(&NamedType {
                name: "SocketAddr",
                ty: &DataModelType::Enum(&[
                    &NamedVariant {
                        name: "V4",
                        ty: &DataModelVariant::NewtypeVariant(&NamedType {
                            name: "SocketAddrV4",
                                ...
                                &NamedValue {
                                    ...
                                    ty: &NamedType {
                                        ...
                                        ty: &DataModelType::Struct(&[&NamedValue {
                                            name: "octets",
                                            ty: <[u8; 4] as Schema>::SCHEMA,
                                        }]),
                                    },
                                },
                                &NamedValue {
                                    name: "port",
                                    ty: <u16 as Schema>::SCHEMA,
                                },
                            ]),
                        }),
                    },
...
#[derive(Debug, Clone, PartialEq, postcard_schema::Schema, Serialize, Deserialize)]
...
pub enum DebugProbeKind {
    ...
    SocketAddr(SocketAddrWrapper),
    #[cfg_attr(target_family = "unix", doc = "Unix socket device.")]
...
/// );
/// ```
#[derive(Debug, Clone, Serialize, PartialEq, Eq, Hash, postcard_schema::Schema)]
...
impl TryFrom<&str> for DebugProbeSelector {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        if let Some(rest) = value.strip_prefix(Self::NET_PFX) {
            let net = rest.parse::<SocketAddr>().map(SocketAddrWrapper)?;
...
mod test {
    ...
    fn test_parsing_net_pfx_v4() {
        ...
        assert_eq!(
            selector,
            DebugProbeSelector::SocketAddr(SocketAddrWrapper(SocketAddr::V4(SocketAddrV4::new(
    ...
    fn test_parsing_net_pfx_v6() {
        ...
        assert_eq!(
            selector,
            DebugProbeSelector::SocketAddr(SocketAddrWrapper(SocketAddr::V6(SocketAddrV6::new(
    ...
    #[cfg_attr(target_family = "unix", test)]
    #[cfg(target_family = "unix")]
