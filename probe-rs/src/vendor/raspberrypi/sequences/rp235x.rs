...
/// This magic value must be written to SCRATCH5, and is equal to -WATCHDOG_MAGIC.
...
impl ArmDebugSequence for Rp235x {
    fn reset_system(
        ...
        // Drop to a safe speed for both AIRCR and rescue reset sequences. At
        // high speeds the AP transactions after a reset arrive before the
        // RP2350's debug subsystem has reinitialized, causing FAULT on DRW.
        // Running at a low speed naturally spaces transactions far enough
        // apart that the chip always has time to recover.
        let probe_speed = {
            let arm_interface = core.get_arm_debug_interface()?;
            arm_interface.try_dap_probe_mut().map(|probe| {
        ...
        // If RP_AIRCR_RESET is set, attempt an AIRCR reset first. This preserves
        // the QMI flash configuration set up by the BootROM, giving the
        // best programming speed. However it does not guarantee that core 1 is
        // halted if it is running, which could in rare cases corrupt memory. Only
        // enable this if you are confident core 1 is not running, or accept the
        // small risk of a memory glitch.
        if matches!(
            std::env::var("RP_AIRCR_RESET").as_deref(),
            Ok("1") | Ok("true") | Ok("yes")
        ) {
            ...
            core.write_word_32(RAM_ADDRESS, 0x2350_2350)?;
            let readback = core.read_word_32(RAM_ADDRESS)?;
            if readback == 0x2350_2350 {
        ...
        // Put the SoC into rescue reset as described in the datasheet.
        //
        // This works by setting a flag in the CTRL register of the RP-AP,
        // which causes a flag to be set in POWMAN CHIP_RESET (RESCUE) and
        // the SoC to reset. The BootROM checks for the RESCUE flag and, if
        // set, halts the system so we can attach as usual.
        //
