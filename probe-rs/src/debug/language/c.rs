...
fn read_signed_int(
    variable: &Variable,
    memory: &mut dyn MemoryInterface,
) -> Result<isize, DebugError> {
    let unsigned = read_unsigned_int(variable, memory)?;
    // sign extend
    let shift = (std::mem::size_of::<isize>() - variable.byte_size.unwrap_or(1) as usize) * 8;
    Ok((unsigned << shift) as isize >> shift)
}

fn read_f32(variable: &Variable, memory: &mut dyn MemoryInterface) -> Result<f32, DebugError> {
    let mut buff = [0u8; 4];
