    ...
    let new_value = match register_rule {
        Undefined => {
            ...
            match &debug_register {
                fp if fp.core_register.roles == Some(RegisterRole::FramePointer) => {
                    register_rule_string = "FP=CFA (dwarf Undefined)".to_string();
                    callee_frame_registers
                        .get_frame_pointer()
                        .and_then(|fp| fp.value)
                }
                sp if sp.core_register.roles == Some(RegisterRole::StackPointer) => {
                ...
                lr if lr.core_register.roles == Some(RegisterRole::ReturnAddress) => {
                ...
                pc if pc.core_register.roles == Some(RegisterRole::ProgramCounter) => {
        ...
        Offset(address_offset) => {
            ...
            if let Some(unwind_cfa) = unwind_cfa {
                ...
                match result {
                    Ok(register_value) => {
                        if debug_register.core_register.roles == Some(RegisterRole::ReturnAddress) {
