...
pub(crate) static RISCV_REGISTERS: Lazy<RegisterFile> =
    Lazy::new(|| RegisterFile::new((*RISCV_REGISTERS_VEC).to_vec()));

static RISCV_REGISTERS_VEC: Lazy<Vec<CoreRegister>> = Lazy::new(|| {
    vec![
        CoreRegister {
            name: "x0",
