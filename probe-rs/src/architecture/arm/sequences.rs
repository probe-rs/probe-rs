...
/// The default sequences that is used for chips using the ARM Debug Interface that do not specify a specific sequence.
#[derive(Debug)]
...
/// The default sequences that is used for ARM cores that do not specify a specific sequence.
#[derive(Debug)]
pub struct DefaultArmCoreSequence(pub(crate) ());

impl DefaultArmCoreSequence {
    /// Creates a new default ARM debug sequence.
    pub fn create() -> Arc<dyn ArmCoreDebugSequence> {
        Arc::new(Self(()))
    }
}
...
impl ArmCoreDebugSequence for DefaultArmSequence {}

impl ArmCoreDebugSequence for DefaultArmCoreSequence {}

/// A interface to operate debug sequences for ARM cores.
///
/// This is used for Cortex-M and Cortex-A cores.
///
/// Should be implemented on a custom handle for chips that require special sequence code.
pub trait ArmCoreDebugSequence: Send + Sync + Debug {
    ...
    fn debug_core_start(
        ...
        match core_type {
            CoreType::Armv7a => armv7a::core_start(&mut *core, debug_base),
            CoreType::Armv8a => armv8a::core_start(&mut *core, debug_base, cti_base),
    ...
    fn reset_catch_set(
        ...
        // Dispatch based on core type (Cortex-A vs M)
        match core_type {
            CoreType::Armv7a => armv7a::reset_catch_set(core, debug_base),
            CoreType::Armv8a => armv8a::reset_catch_set(core, debug_base),
            CoreType::Armv6m | CoreType::Armv7m | CoreType::Armv7em | CoreType::Armv8m => {
                cortex_m::reset_catch_set(core)
            }
            _ => panic!("Logic inconsistency bug - non ARM core type passed {core_type:?}"),
        }
    }
    ...
    fn reset_catch_clear(
        ...
        // Dispatch based on core type (Cortex-A vs M)
        match core_type {
            CoreType::Armv7a => armv7a::reset_catch_clear(core, debug_base),
            CoreType::Armv8a => armv8a::reset_catch_clear(core, debug_base),
            CoreType::Armv6m | CoreType::Armv7m | CoreType::Armv7em | CoreType::Armv8m => {
                cortex_m::reset_catch_clear(core)
            }
            _ => panic!("Logic inconsistency bug - non ARM core type passed {core_type:?}"),
        }
    }
    ...
    fn reset_system(
        ...
        // Dispatch based on core type (Cortex-A vs M)
        match core_type {
            CoreType::Armv7a => armv7a::reset_system(interface, debug_base),
            CoreType::Armv8a => armv8a::reset_system(interface, debug_base),
            CoreType::Armv6m | CoreType::Armv7m | CoreType::Armv7em | CoreType::Armv8m => {
                cortex_m::reset_system(interface)
            }
            _ => panic!("Logic inconsistency bug - non ARM core type passed {core_type:?}"),
        }
    }
...
///
/// Should be implemented on a custom handle for chips that require special sequence code.
pub trait ArmDebugSequence: ArmCoreDebugSequence + Send + Sync + Debug {
