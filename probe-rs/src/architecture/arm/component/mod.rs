...
pub fn setup_tracing(core: &mut Core, romtable: &RomTable) -> Result<(), Error> {
    // stm32 specific reg (DBGMCU_CR):
    core.write_word_32(0xE004_2004, 0x27)?;

    // Config tpiu:
    let mut tpiu = romtable.tpiu(core).map_err(Error::architecture_specific)?;
    ...
    // Config itm:
    let mut itm = romtable.itm(core).map_err(Error::architecture_specific)?;
    itm.unlock()?;
    itm.tx_enable()?;
