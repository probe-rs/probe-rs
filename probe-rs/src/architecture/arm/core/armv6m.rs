...
use crate::{
    ...
    HaltReason, InstructionSet, MemoryInterface, MemoryMappedRegister, RegisterLocation,
};
use anyhow::Result;
...
impl<'probe> CoreInterface for Armv6m<'probe> {
    ...
    fn halt(&mut self, timeout: Duration) -> Result<CoreInformation, Error> {
        ...
        let pc_value = self.read_core_reg(PC.location)?;
    ...
    fn reset_and_halt(&mut self, _timeout: Duration) -> Result<CoreInformation, Error> {
        ...
        let xpsr_value: u32 = self.read_core_reg(XPSR.location)?.try_into()?;
        if xpsr_value & XPSR_THUMB == 0 {
            self.write_core_reg(XPSR.location, (xpsr_value | XPSR_THUMB).into())?;
        }
