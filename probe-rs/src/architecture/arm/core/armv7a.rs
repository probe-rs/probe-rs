...
impl<'probe> Armv7a<'probe> {
    ...
    fn prepare_r0_for_clobber(&mut self) -> Result<(), Error> {
        if self.state.register_cache[0].is_none() {
            ...
            let r0_val: u32 = self.read_core_reg(RegisterNumber(0))?.try_into()?;
...
impl<'probe> CoreInterface for Armv7a<'probe> {
    ...
    fn step(&mut self) -> Result<CoreInformation, Error> {
        ...
        let current_pc: u32 = self
            .read_core_reg(register::PC.register_number)?
            .try_into()?;
...
mod test {
    ...
    fn armv7a_read_core_reg_common() {
        ...
        assert_eq!(
            ...
            armv7a.read_core_reg(RegisterNumber(2)).unwrap()
        );
    ...
    fn armv7a_read_core_reg_pc() {
        ...
        assert_eq!(
            RegisterValue::from(REG_VALUE),
            armv7a.read_core_reg(RegisterNumber(15)).unwrap()
        );
    ...
    fn armv7a_read_core_reg_cpsr() {
        ...
        assert_eq!(
            RegisterValue::from(REG_VALUE),
            armv7a.read_core_reg(RegisterNumber(16)).unwrap()
        );
