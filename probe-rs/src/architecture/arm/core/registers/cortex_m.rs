...
pub(crate) static CORTEX_M_COMMON_REGS: Lazy<RegisterFile> = Lazy::new(|| {
    RegisterFile::new(
        (*ARM32_COMMON_REGS_VEC)
            .iter()
            .chain(&(*CORTEX_M_COMMON_REGS_VEC))
            .cloned()
            .collect(),
    )
});

pub(crate) static CORTEX_M_WITH_FP_REGS: Lazy<RegisterFile> = Lazy::new(|| {
    RegisterFile::new(
        (*ARM32_COMMON_REGS_VEC)
            .iter()
            .chain(&(*CORTEX_M_COMMON_REGS_VEC))
            ...
            .collect::<Vec<_>>()
            .iter()
            .chain(&(*CORTEX_M_WITH_FP_REGS_VEC))
            .cloned()
            .collect(),
    )
});

pub(super) static ARM32_COMMON_REGS_VEC: Lazy<Vec<CoreRegister>> = Lazy::new(|| {
    vec![
        CoreRegister {
            name: "R0",
...
static CORTEX_M_COMMON_REGS_VEC: Lazy<Vec<CoreRegister>> = Lazy::new(|| {
    vec![
        CoreRegister {
            name: "MSP",
...
static CORTEX_M_WITH_FP_REGS_VEC: Lazy<Vec<CoreRegister>> = Lazy::new(|| {
    vec![
        CoreRegister {
            name: "FPSCR",
