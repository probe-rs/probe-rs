...
impl<'probe> CoreInterface for Armv7m<'probe> {
    ...
    fn run(&mut self) -> Result<(), Error> {
        ...
        let mut dhcsr = Dhcsr(self.memory.read_word_32(Dhcsr::get_mmio_address(None))?);
        ...
        if dhcsr.c_maskints() {
            ...
            dhcsr.enable_write();
            self.memory
                .write_word_32(Dhcsr::get_mmio_address(None), dhcsr.into())?;
            self.memory.flush()?;
        }
    ...
    fn set_hw_breakpoint(&mut self, bp_unit_index: usize, addr: u64) -> Result<(), Error> {
        ...
        let reg_addr =
            FpRev1CompX::get_mmio_address(None) + (bp_unit_index * size_of::<u32>()) as u64;
    ...
    fn hw_breakpoints(&mut self) -> Result<Vec<Option<u64>>, Error> {
        ...
        { 0..num_hw_breakpoints }.try_for_each(|bp_unit_index| {
            ...
            let reg_addr = FpRev1CompX::get_mmio_address(None) + (bp_unit_index * size_of::<u32>()) as u64;
