...
impl ImageLoader for IdfLoader {
    async fn load(
        ...
        let flash_size_result = session
            .halted_access(
                async |session| match session.target().debug_sequence.clone() {
...
impl FlashLoader {
        ...
        if let Some(instr_set) = image_instruction_set {
            ...
            for (core, _) in session.list_cores() {
                match session.core(core).await {
                    Ok(mut core) => {
                        if let Ok(set) = core.instruction_set().await {
    ...
    async fn initialize(
        ...
        for flasher in algos.iter() {
            ...
            if options.do_chip_erase && !flasher.is_chip_erase_supported(session).await {
