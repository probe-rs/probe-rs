...
impl Flasher {
    ...
    async fn ensure_loaded(&mut self, session: &mut Session) -> Result<(), FlashError> {
        if !self.loaded {
    ...
    async fn load(&mut self, session: &mut Session) -> Result<(), FlashError> {
    ...
    pub(super) async fn init<'s, 'p, O: Operation>(
        ...
        self.ensure_loaded(session).await?;
    ...
    pub(super) async fn run_erase_all(
        ...
        let result = if session.has_sequence_erase_all().await {
            session
                .sequence_erase_all()
                .await
    ...
    pub(super) async fn is_chip_erase_supported(&self, session: &Session) -> bool {
        session.has_sequence_erase_all().await || self.flash_algorithm().pc_erase_all.is_some()
    }
    ...
    pub(super) async fn fill_unwritten(
        ...
        let result = if self.flash_algorithm.pc_read.is_some() {
            ...
            let mut core = session.core(0).await.map_err(FlashError::Core)?;
            fill_pages(&mut self.regions, progress, async |address, data| {
                core.read(address, data).await.map_err(FlashError::Core)
            })
            .await
        };
        ...
        if let Some(verify) = self.flash_algorithm.pc_verify {
            ...
            if self.flash_algorithm.pc_read.is_some() {
                ...
                compare_flash(
                    ...
                    progress,
                    ignore_filled,
                    async |address, data| core.read(address, data).await.map_err(FlashError::Core),
                )
                .await
            }
        }
