...
enum Cli {
    ...
    /// Regenerate DAP Rust types from the checked-in `debugProtocol.json` using schemafy.
    UpdateDapSchema,
}
...
fn try_main() -> anyhow::Result<()> {
    match Cli::parse() {
        ...
        Cli::UpdateDapSchema => update_dap_schema()?,
    }

    Ok(())
}

const DAP_SCHEMA_JSON: &str =
    "probe-rs-tools/src/bin/probe-rs/cmd/dap_server/debug_adapter/dap/debugProtocol.json";
const DAP_SCHEMA_RS: &str =
    "probe-rs-tools/src/bin/probe-rs/cmd/dap_server/debug_adapter/dap/debugProtocol.rs";
...
fn update_dap_schema() -> Result<()> {
    let root = workspace_root();
    let schema_json = root.join(DAP_SCHEMA_JSON);
    let schema_rs = root.join(DAP_SCHEMA_RS);
    ...
    println!("Generating Rust types from {}", schema_json.display());
    let tokens = schemafy_lib::Generator::builder()
        .with_root_name_str("debugserver_types")
        .with_input_file(&schema_json)
    ...
    writeln!(generated)?;
    writeln!(generated, "#![allow(clippy::all)]")?;
    writeln!(generated, "#![allow(dead_code, unused_imports)]")?;
    writeln!(generated)?;
    writeln!(generated, "use serde::{{Deserialize, Serialize}};")?;
    writeln!(generated)?;
    write!(generated, "{tokens}")?;

    std::fs::write(&schema_rs, generated.as_bytes())
        .with_context(|| format!("Failed to write {}", schema_rs.display()))?;

    let sh = Shell::new()?;
    cmd!(sh, "rustfmt --edition 2024 {schema_rs}")
        .run()
        .with_context(|| format!("Failed to rustfmt {}", schema_rs.display()))?;

    println!("Wrote {}", schema_rs.display());
    Ok(())
}
