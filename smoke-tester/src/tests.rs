...
pub async fn test_register_write(tracker: &TestTracker<'_>, core: &mut Core<'_>) -> TestResult {
    ...
    for register in register.core_registers() {
        ...
        let readback: u64 = core.read_core_reg(register).await.into_diagnostic()?;
...
pub async fn test_memory_access(tracker: &TestTracker<'_>, core: &mut Core<'_>) -> TestResult {
    ...
    for ram in memory_regions {
        ...
        let value = core.read_word_32(ram_start).await.into_diagnostic()?;
        assert_eq!(
            value, 0xababab,
        ...
        core.write_word_32(addr, 0xababac).await.into_diagnostic()?;
        let value = core.read_word_32(addr).await.into_diagnostic()?;
        assert_eq!(
            value, 0xababac,
            "Error reading back 4 bytes from address {:#010X}",
            addr
        ...
        // Write first byte
        core.write_word_8(ram_start, 0xac).await.into_diagnostic()?;
        let value = core.read_word_8(ram_start).await.into_diagnostic()?;
        assert_eq!(
            value, 0xac,
...
pub async fn test_hw_breakpoints(tracker: &TestTracker<'_>, core: &mut Core<'_>) -> TestResult {
    ...
    for region in memory_regions {
        ...
        let num_breakpoints = core.available_breakpoint_units().await.into_diagnostic()?;
        ...
        let breakpoints = core.hw_breakpoints().await.into_diagnostic()?;
        for (i, address) in breakpoint_addresses.iter().enumerate() {
            assert_eq!(
                breakpoints[i],
                Some(*address),
